...
class FrozenCLIPEmbedderWrapper(AbstractEncoder):
    ...
    def __init__(self, modifier_token, version="openai/clip-vit-large-patch14", device="cuda", max_length=77):
        ...
        def _hf_resolve(repo_id: str, cache_root: str) -> str:
            # 1) Try cache-only first (no network).
            try:
                _ = CLIPTokenizer.from_pretrained(repo_id, cache_dir=cache_root, local_files_only=True)
                _ = CLIPTextModel.from_pretrained(repo_id, cache_dir=cache_root, local_files_only=True)
                return repo_id  # ok to load by id from cache
            except Exception:
                pass

            # 2) If not cached, download snapshot into cache_root (uses network).
        ...
        self.transformer = CLIPTextModel.from_pretrained(source, cache_dir=cache_root, local_files_only=True)
    ...
    def forward(self, text):
        ...
        embed_device = self.transformer.text_model.embeddings.token_embedding.weight.device
        tokens = tokens.to(embed_device)
        indices = indices.to(embed_device)
